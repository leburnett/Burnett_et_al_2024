function process_HC_LC(folder_to_analyze, rftiming, drf, istart, iend, ri2, save_str)
...
% interval in sec*sampling_rate between 2 frames
    ...
    if save_str == "HC" || save_str == "LC"
        
        av_hist = zeros(n_gcl,allframesnum-1);
        for ci=1:n_gcl
            av_hist(ci,:)=mean(squeeze(all_spikes(ci,:,:)));
        end
%         save(strcat(folder_to_analyze,'/Spikes_', save_str, '.mat'), 'all_spikes', 'av_hist');
        
        figure, imagesc(av_hist)
        hold on
        plot([45 45], [-100 200], 'w')
        plot([90 90], [-100 200], 'w')
        plot([135 135], [-100 200], 'w')
        plot([180 180], [-100 200], 'w')
        plot([225 225], [-100 200], 'w')
        axis([0 250 0 n_gcl+1])
        colorbar
        caxis([0 2.5])
        
    elseif save_str == "DS"
        SP1_hist = squeeze(all_spikes(:,1,1:250));
        SP2_hist = squeeze(all_spikes(:,2,1:250));
        SP3_hist = squeeze(all_spikes(:,3,1:250));
        SP4_hist = squeeze(all_spikes(:,4,1:250));
        SP5_hist = squeeze(all_spikes(:,5,1:250));
        
%         save(strcat(folder_to_analyze,'/Spikes_', save_str, '.mat'), 'all_spikes', 'SP1_hist', 'SP2_hist', 'SP3_hist', 'SP4_hist', 'SP5_hist'); 
...
% save(strcat(folder_to_analyze, save_str, '.mat'), 'all_spikes', 'av_hist'); 
